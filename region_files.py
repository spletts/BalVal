...
# BalVal #
import outputs
from catalog_headers import MATCH_TYPE, RA_HDR1, RA_HDR2, DEC_HDR1, DEC_HDR2, MAJOR_AX_HDR1, MAJOR_AX_HDR2, ANGLE_HDR1, ANGLE_HDR2, CM_T_HDR1, CM_T_HDR2
...
def write_region_file(ra, dec, fn, color, shape, overwrite, a=None, b=None, radius=None, angle=None):
...
def write_region_files(df_1and2, df_1not2, df_2not1, realization, tile, balrog_run, output_directory):
	...
	if 'coadd' in (MATCH_CAT1, MATCH_CAT2) or 'y3_gold' in MATCH_CAT1 or 'y3_gold' in MATCH_CAT2:
		...
		orientation_1and2, orientation_1not2, orientation_2not1 = df_1and2[ANGLE_HDR1], df_1not2[ANGLE_HDR1], df_2not1[ANGLE_HDR1]

		fnRegion1and2 = write_region_file(ra=ra_1and2, dec=dec_1and2, fn=fnRegion1and2, color='green', shape='ellipse', a=a_1and2, b=b_1and2, radius=None, angle=orientation_1and2, overwrite=__overwrite)
		fnRegion1not2 = write_region_file(ra=ra_1not2, dec=dec_1not2, fn=fnRegion1not2, color='blue', shape='ellipse', a=a_1not2, b=b_1not2, radius=None, angle=orientation_1not2, overwrite=__overwrite)
		fnRegion2not1 = write_region_file(ra=ra_2not1, dec=dec_2not1, fn=fnRegion2not1, color='yellow', shape='ellipse', a=a_2not1, b=b_2not1, radius=None, angle=orientation_2not1, overwrite=__overwrite)


        if 'coadd' not in (MATCH_CAT1, MATCH_CAT2) and 'y3_gold' not in MATCH_CAT1 and 'y3_gold' not in MATCH_CAT2:
		...
		fnRegion1and2 = write_region_file(ra=ra_1and2, dec=dec_1and2, fn=fnRegion1and2, color='green', shape='cirlce', radius=size_1and2, overwrite=__overwrite)
                fnRegion1not2 = write_region_file(ra=ra_1not2, dec=dec_1not2, fn=fnRegion1not2, color='blue', shape='circle', radius=size_1not2, overwrite=__overwrite)
                fnRegion2not1 = write_region_file(ra=ra_2not1, dec=dec_2not1, fn=fnRegion2not1, color='yellow', shape='circle', radius=size_2not1, overwrite=__overwrite)	
