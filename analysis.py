...
def get_coadd_catalog_flags(df_1and2, suf, band):
    ...
    __flags = []
    flag1 = get_floats_from_string(df=df_1and2, band=band, four_elmt_arrs_hdr='{}{}'.format(NEW_COADD_FLAG_GRIZ_HDR, suf))
    flag2 = get_floats_from_string(df=df_1and2, band=band, four_elmt_arrs_hdr='{}{}'.format(NEW_COADD_IMAFLAG_ISO_GRIZ_HDR, suf))
...
def get_additional_flags(cat_type, suf, band, df_1and2):
    """
    ...
    if cat_type == 'coadd':
        flags = get_coadd_catalog_flags(df_1and2=df_1and2, suf=suf, band=band)

    if 'y3_gold' in cat_type:
        flags = get_y3_gold_catalog_flags(suf=suf, band=band, df_1and2=df_1and2)
...
def get_good_indices_using_primary_flags(df_1and2, full_mag1, full_mag2, cm_flag_hdr1, cm_flag_hdr2, flag_hdr1, flag_hdr2, band=None):
    ...
    if MATCH_CAT1 == 'coadd' or 'y3_gold' in MATCH_CAT1:
        flags_list1 = get_additional_flags(cat_type=MATCH_CAT1, suf='_1', band=band, df_1and2=df_1and2)
        __num_additional_flags += len(flags_list1)

    if MATCH_CAT2 == 'coadd' or 'y3_gold' in MATCH_CAT2:
        flags_list2 = get_additional_flags(cat_type=MATCH_CAT2, suf='_2', band=band, df_1and2=df_1and2)
    ...
    try:
        flags_list1
        for i in np.arange(0, len(flags_list1)):
    ...
    try:
        flags_list2
        for j in np.arange(0, len(flags_list2)):
...
def bin_and_cut_measured_magnitude_error(clean_mag1, clean_mag2, mag_err1, mag_err2, band, tile, realization, fn_mag_err_log, fn_mag_diff_outliers_log):
    ...
    Remove error values corresponding to objects with |MagnitudeDifference|>cutoff. 
    Do not consider error corresponding to empty bins nor bins with a small number of objects.
...
def one_sigma_magnitude_counter_for_normalized_magnitude_difference(full_mag, norm_mag_diff, clean_mag, mag_bins_for_mag_err, hax_mag, mag_err_bin_medians, norm_vax_mag_bin_medians, tile, realization, band, fn_num_objs_in_1sig_mag_log):
    ...
    with open(fn_num_objs_in_1sig_mag_log, 'a') as csvfile:
        writer = csv.writer(csvfile, delimiter=',')
        writer.writerow([tile, realization, band, len(full_mag), len(full_mag)-len(clean_mag) , __num_1sigma_mag_norm, str(NORMALIZE_MAG), str(RUN_TYPE)])
    csvfile.close()
...
def get_magnitude_error(mag_err_hdr, flux_hdr, flux_cov_hdr, df_1and2, band, idx_good, match_cat):
    ...
    if PLOT_MAG_ERR and 'truth' not in match_cat:
        ...
        if mag_err_hdr is not None:
            if match_cat == 'coadd':
            #TODO get_dataframe should assing mag_err_hdr
                try:
                    __mag_error = get_floats_from_string(df=df_1and2, four_elmt_arrs_hdr='{}_1'.format(NEW_COADD_MAG_ERR_GRIZ_HDR), band=band)
                except:
                    __mag_error = get_floats_from_string(df=df_1and2, four_elmt_arrs_hdr='{}_2'.format(NEW_COADD_MAG_ERR_GRIZ_HDR), band=band)
...
def get_magnitude_completeness(idx_good, df_1and2, clean_mag1, clean_mag2, full_mag1, full_mag2, tile, realization, band, mag_hdr1, mag_hdr2, mag_err1, mag_err2, fn_mag_completeness_log, inj_percent, balrog_run, base_path_to_catalogs): #FIXME inj?
    ...
    __truth_mag = np.empty(len(truth_mag_griz))

    for i in np.arange(0, len(truth_mag_griz)):
        __truth_mag[i] = truth_mag_griz[i][BAND_INDEX[band]]
...
def get_coadd_catalog_observables(fn_coadd_cat_g, fn_coadd_cat_r, fn_coadd_cat_i, fn_coadd_cat_z, mag_hdr, mag_err_hdr, flux_hdr, flux_err_hdr):
    ...
    __coadd_flux_griz, __coadd_flux_err_griz, __coadd_mag_griz, __coadd_mag_err_griz = np.empty(len(m_g), dtype='S100'), np.empty(len(m_g), dtype='S100'), np.empty(len(m_g), dtype='S100'), np.empty(len(m_g), dtype='S100')
...
def get_star_truth_catalog_magnitude(df_1and2, suf):
    ...
    __star_truth_mag_griz = np.empty(len(m_g), dtype='S100')
...
def get_y3_gold_catalog_magnitude(df_1and2, mag_hdr):
    ...
    __y3_gold_mag_griz = np.empty(len(m_g), dtype='S100')
...
def get_stacked_magnitude_differences(realization, mag_hdr1, mag_hdr2, mag_err_hdr1, mag_err_hdr2):
    """Store magnitude differences of objects in multiple tiles as an `np.array()` rather than create a stacked catalog.
